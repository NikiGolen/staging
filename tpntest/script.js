...
function renderRisks(program) {
    ...
    // Composite severity: weighted by how many risks fall in each
    // probability x impact cell, not an arbitrary flat formula.
    const weights = { Low: 1, Medium: 2, High: 3 };

    const total = program.risks.reduce((sum, risk) => {
        return sum + weights[risk.probability] * weights[risk.impact];
    }, 0);

    const maxPossible = program.risks.length * 9; // 3 x 3 ceiling per risk
    const score = maxPossible ? Math.round((total / maxPossible) * 100) : 0;
